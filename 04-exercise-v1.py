...
for i in range(0,len(tulpeList)):
    v = dice.setdefault(sumTulpeList[i],set()) # przypisuje na zmienną v wartości słownika o kluczu (key) równym sumie oczek z dwóch rzutów (i-ta kolejka). Jeśli w kluczach słownika dice nie było do tej pory tego klucza to przypisuje na v wartość default = "set()" "zbiór pusty"
    print(v)
